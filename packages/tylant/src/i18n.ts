...
export function processI18nConfig(
    ...
    return { astroI18nConfig: getAstroI18nConfig(tylantConfig), tylantConfig: tylantConfig };
}

/** Generate an Astro i18n configuration based on a Starlight configuration. */
function getAstroI18nConfig(config: TylantConfig): NonNullable<AstroConfig['i18n']> {
    return {
...
): Pick<TylantConfig, 'isMultilingual' | 'locales' | 'defaultLocale'> {
